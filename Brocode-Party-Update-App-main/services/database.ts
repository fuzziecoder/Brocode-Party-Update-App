...
import { Spot, Invitation, Payment, InvitationStatus, PaymentStatus, UserProfile, Drink, Attendance, Cigarette, Food, Notification, DrinkBrand, UserDrinkSelection, Moment, SpotSponsor } from '../types';
...
export const sponsorService = {
  ...
  async getSponsor(spotId: string): Promise<SpotSponsor | null> {
    const { data, error } = await supabase
      ...
      .select(`
        *,
        profiles:sponsor_id (
          id, name, username, profile_pic_url, is_sponsor, sponsor_count
        )
      `)
      .eq('spot_id', spotId)
      .maybeSingle();
  ...
  async sponsorSpot(data: {
    ...
    if (error) {
      console.error('Error sponsoring spot:', error);
      throw error;
    }
    return sponsor;
  },

  // Remove sponsor from a spot
  async removeSponsor(spotId: string): Promise<void> {
    ...
    // Also reset the spot flags
    await supabase
      .from('spots')
      .update({ is_sponsored: false, sponsored_by: null })
      .eq('id', spotId);
  },

  // Get all sponsors (for leaderboard)
  async getAllSponsors(): Promise<SpotSponsor[]> {
    const { data, error } = await supabase
      .from('spot_sponsors')
      .select(`
        *,
  ...
  subscribeToSponsors(spotId: string, callback: (payload: any) => void) {
    return supabase
      .channel(`sponsors-${spotId}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'spot_sponsors', filter: `spot_id=eq.${spotId}` },
        callback
      )
      .subscribe();
  },
...
export const billService = {
  // Admin confirms a user's bill
  async confirmBill(spotId: string, userId: string): Promise<void> {
    const { error } = await supabase
      .from('payments')
      .update({ bill_confirmed: true })
      .eq('spot_id', spotId)
      .eq('user_id', userId);
  ...
  // Admin un-confirms a user's bill
  async unconfirmBill(spotId: string, userId: string): Promise<void> {
    const { error } = await supabase
      .from('payments')
      .update({ bill_confirmed: false })
      .eq('spot_id', spotId)
      .eq('user_id', userId);

    if (error) {
      console.error('Error un-confirming bill:', error);
      throw error;
    }
  },

  // Get bill confirmation status for a user
  async getBillConfirmation(spotId: string, userId: string): Promise<boolean> {
    const { data, error } = await supabase
      .from('payments')
      .select('bill_confirmed')
      .eq('spot_id', spotId)
      .eq('user_id', userId)
      .maybeSingle();

    if (error) {
      console.error('Error fetching bill confirmation:', error);
      return false;
    }
    return data?.bill_confirmed ?? false;
  },

  // Get all bill confirmations for a spot
  async getAllBillConfirmations(spotId: string): Promise<Record<string, boolean>> {
    const { data, error } = await supabase
      .from('payments')
      .select('user_id, bill_confirmed')
      .eq('spot_id', spotId);

    if (error) {
      console.error('Error fetching bill confirmations:', error);
      return {};
    }

    const map: Record<string, boolean> = {};
    (data || []).forEach((p: any) => {
      map[p.user_id] = p.bill_confirmed ?? false;
    });
    return map;
  },
};
