...
interface GlowButtonProps extends Omit<React.ComponentProps<typeof motion.button>, 'children'> {
  ...
  variant?: 'primary' | 'danger' | 'secondary' | 'sponsor';
  size?: 'sm' | 'md' | 'lg';
}

const GlowButton: React.FC<GlowButtonProps> = ({ children, variant = 'primary', size = 'md', className = '', ...props }) => {
  ...
  const sponsorGlow = "from-yellow-400 via-amber-500 to-orange-500";

  const glowGradient =
    variant === 'primary' ? primaryGlow :
      variant === 'danger' ? dangerGlow :
        variant === 'sponsor' ? sponsorGlow :
          secondaryGlow;
  ...
  return (
    ...
    >
      ...
      <div className={`relative w-full h-full ${sizeClasses[size]} transition-all ease-in duration-150 bg-black/80 backdrop-blur-md border border-white/10 rounded-full group-hover:bg-black/40`}>
        <span className="relative text-white whitespace-nowrap flex items-center justify-center overflow-hidden">{children}</span>
      </div>
    </motion.button>
  );
};
